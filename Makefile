...
.PHONY: proto coordinator_proto automations_proto

REQUIRED_PROTOC_PREFIX ?=
proto_check:
	@if ! command -v $(PROTOC) >/dev/null 2>&1 ; then \
		echo "ERROR: protoc not found on PATH"; exit 1; \
	fi
	@if [ -n "$(REQUIRED_PROTOC_PREFIX)" ]; then \
		V="$$($(PROTOC) --version | awk '{print $$2}')"; \
		case "$$V" in \
			$(REQUIRED_PROTOC_PREFIX)*) echo "protoc $$V (matches prefix $(REQUIRED_PROTOC_PREFIX))";; \
			*) echo "ERROR: protoc $$V does not match required prefix $(REQUIRED_PROTOC_PREFIX)"; exit 1;; \
		esac \
	fi
...
COORDINATOR_PROTO_DIR := ./internal/coordinator/proto
COORDINATOR_PB_DIR := ./internal/coordinator/pb
COORDINATOR_PROTO_FILES := $(wildcard $(COORDINATOR_PROTO_DIR)/*.proto)
COORDINATOR_EXTRA_INCLUDES ?=
COORDINATOR_INCLUDE_FLAGS := -I $(COORDINATOR_PROTO_DIR)
ifneq ($(strip $(COORDINATOR_EXTRA_INCLUDES)),)
  COORDINATOR_INCLUDE_FLAGS += $(foreach inc,$(subst ',', ,$(COORDINATOR_EXTRA_INCLUDES)),-I $(inc))
endif
COORDINATOR_PROTOS := $(shell find $(COORDINATOR_PROTO_DIR) -type f -name '*.proto')
coordinator_proto:
	@echo ">> Generating Go code into $(COORDINATOR_PB_DIR)"
	@mkdir -p $(COORDINATOR_PB_DIR)
	@$(PROTOC) $(COORDINATOR_INCLUDE_FLAGS) \
		--proto_path=. \
		--go_out=$(COORDINATOR_PB_DIR) \
		--go_opt=paths=source_relative \
		--go-grpc_out=$(COORDINATOR_PB_DIR) \
		--go-grpc_opt=paths=source_relative \
		$(COORDINATOR_PROTOS)
	@echo ">> Done."


AUTOMATIONS_PROTO_DIR := ./internal/automations/proto
AUTOMATIONS_PB_DIR := ./internal/automations/pb
AUTOMATIONS_PROTO_FILES := $(wildcard $(AUTOMATIONS_PROTO_DIR)/*.proto)
AUTOMATIONS_EXTRA_INCLUDES ?=
AUTOMATIONS_INCLUDE_FLAGS := -I $(AUTOMATIONS_PROTO_DIR)
ifneq ($(strip $(AUTOMATIONS_EXTRA_INCLUDES)),)
  AUTOMATIONS_INCLUDE_FLAGS += $(foreach inc,$(subst ',', ,$(AUTOMATIONS_EXTRA_INCLUDES)),-I $(inc))
endif
AUTOMATIONS_PROTOS := $(shell find $(AUTOMATIONS_PROTO_DIR) -type f -name '*.proto')
automations_proto:
	@echo ">> Generating Go code into $(AUTOMATIONS_PB_DIR)"
	@mkdir -p $(AUTOMATIONS_PB_DIR)
	@$(PROTOC) $(AUTOMATIONS_INCLUDE_FLAGS) \
		--proto_path=. \
		--go_out=$(AUTOMATIONS_PB_DIR) \
		--go_opt=paths=source_relative \
		--go-grpc_out=$(AUTOMATIONS_PB_DIR) \
		--go-grpc_opt=paths=source_relative \
		$(AUTOMATIONS_PROTOS)
	@echo ">> Done."

proto: coordinator_proto automations_proto
