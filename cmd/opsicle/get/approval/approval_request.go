...
var Command = &cobra.Command{
	Use:     "approval <approval-uuid>",
	Aliases: []string{"approv", "app", "a"},
	Short:   "Retrieves an approval its UUID",
	PreRun: func(cmd *cobra.Command, args []string) {
		flags.BindViper(cmd)
	},
	RunE: func(cmd *cobra.Command, args []string) error {
		if len(args) == 0 {
			return fmt.Errorf("failed to receive <approval-uuid")
		}
		approvalUuid := args[0]
		...
		approval, err := client.GetApproval(approvalUuid)
		if err != nil {
			return fmt.Errorf("failed to retrieve approval[%s]: %s", approvalUuid, err)
		}

		o, _ := json.MarshalIndent(approval, "", "  ")
