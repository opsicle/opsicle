...
func CreateUserMfaV1(opts CreateUserMfaV1Opts) (*UserMfa, error) {
	...
	if err != nil {
		return nil, fmt.Errorf("models.CreateUserMfaV1: failed to execute query: %s", err)
	}
	if rowsAffected, err := results.RowsAffected(); err != nil {
		return nil, fmt.Errorf("models.CreateUserMfaV1: failed to get created row: %s", err)
	} else if rowsAffected == 0 {
		return nil, fmt.Errorf("models.CreateUserMfaV1: failed to create a row: %s", err)
	}
